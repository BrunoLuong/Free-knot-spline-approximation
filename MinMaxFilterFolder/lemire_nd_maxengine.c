 ...
 * Compilation:
 *  MSVS C Compiler
 *  >> mex -O -R2018a COMPFLAGS="$COMPFLAGS /openmp" lemire_nd_maxengine.c 
 *  Intel compiler
 *  >> mex -O -R2018a COMPFLAGS="$COMPFLAGS /MD /Qopenmp" lemire_nd_maxengine.c 
 *
...
__pragma(omp parallel for default(none) private(i, j, k) \
    private(a, idx, maxval, maxidx, Wedge) \
    private(nWedge, Wedgefirst, Wedgelast, left, pleft) \
    private(linidx) \
    schedule(static) \
    shared(adata, valdata, stepA, stepMinMax, idxdata, maxidxdata, lstart, \
...
void mexFunction(int nlhs, mxArray *plhs[],
    ...
    if (WedgeBuffer ==NULL) mexErrMsgTxt("LEMIRE_ND_MAXENGINE: out of memory.");
