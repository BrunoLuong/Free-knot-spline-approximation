...
%                  https://gitlab.inria.fr/alta/alta/-/tree/master/sources/matlab
...
%          29-November-2016: quadprog is functioning (blind codding before)
%                            Force Hessian matrix to be invertible before
%                            calling quadprog
...
global BSFK_DISPLAY WAITBARH BSFK_QPENGINE %#ok
% persistent ax1
...
function [alpha, actset, uineq, ueq] = ConL2Fit(y, B, D, LU, X, Deq, veq, Xeq, ...
                                     qpengine)
% [alpha, actset, uineq, ueq] = ConL2Fit(y, B, D, LU, X, Deq, veq, Xeq)
...
% uineq, ueq: Lagrange multiplier
% 

global BSFK_DISPLAY BSFK_QPENGINE %#ok 
...
switch lower(qpengine)
    ...
    case {1,'quadprog'} % mlint does not like mixed type
            ...
            switch ier
                case -2
                    warning('BSFK:QPNonConvergence', ...
                        'ConL2Fit: Problem is infeasible');
                case -3
                    warning('BSFK:QPNonConvergence', ...                       
                        'ConL2Fit: Problem is unbounded');
                case -4
                    warning('BSFK:QPNonConvergence', ...                       
                        'ConL2Fit: Current search direction was not a direction of descent');                   
                case -6
                    warning('BSFK:QPNonConvergence', ...
                        'ConL2Fit: Nonconvex problem detected.');
                otherwise
                    warning('BSFK:QPNonConvergence', ...
                        'ConL2Fit: unknown cause');
            end
        end
        ...
        uineq = X(:,keep)*lbd.ineqlin(:); % X.'*u; but X is symmetric
        ueq = Xeq*ueq; % idem
...
% [t, p, nremoved, validknots, knotidx] = KnotRemoval(x, y, t, k, smoothing, p, ...
%                             firstcall, knotremoval_factor, validknots, ...
%                             shape)
%
...
% dB (m x n x l): Knot-derivative of B
...
function H = RestoreSPMat(H)
% Make presumably SPMat to be numerical SP
...
if size(H,1)>1
    ...
    try
        ...
        %         This error occurs when the input matrix of the EIGS function has a 
        %         very high condition number, which makes EIGS unable to converge to 
        %         the default tolerance of EPS.As a workaround, reduce the tolerance 
        %         of EIGS using the following commands:
